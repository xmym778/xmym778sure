...
from dataloader2 import make_data
# from unet_model.unet_model import UNetModel
...
def cross_validation(args, Train_val_dataset, k_folds=5):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset_indices)):
        ...
        model = AAUNet(n_channels=3,n_classes=1).to(device)
...
def get_args():
    ...
    parser.add_argument('--learning-rate', '-l', metavar='LR', type=float, default=5e-5,
...
if __name__ == '__main__':
    ...
    ROOT_DIR = r"./ph2_dataset"
    train_val_dataset, test_dataset = make_data(ROOT_DIR)
    # 这个dataset时接下来需要做的。
